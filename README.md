...
- 아쉬운 점이 아직 많지만 큼지막한 기능은 모두 구현을 했으므로 나름 성공적인 프로젝트라고 생각합니다.
...
- **지식 부족으로 인해 구현 하지 못한 기능**

  길드 이름 클릭 시 길드원을 볼 수 있는 기능을 구현하고 싶었지만 데이터베이스에 대한 지식 부족으로 구현하지 못했습니다. 이는 백엔드 관련 지식을 쌓은 뒤 추가로 구현 할 예정입니다.
...
- **기획부터 디자인, 개발까지**
  ...
  프로젝트의 기획, 디자인, 개발 모두 경험해 볼 수 있었습니다. 직접 해보니 쉬운게 하나도 없었으며 특히 디자이너의 중요성을 깨닫게 되었습니다.
...
- **컴포넌트 분리와 데이터 가공의 중요성**
  <br />
  
  컴포넌트 분리를 제대로 하지 않으니 재사용성이 떨어지고 불편한 점이 많았습니다. 그리고 데이터를 어떻게 보여줄지 제대로 계획을 하지 않고 보여주려고 하니 제 뜻대로 되지 않아 어려움을 많이 겪었습니다. 그로인해 데이터 가공의 중요성에 대해 깨닫게 되었습니다.

**아직 부족한점이 많아 유지보수와 기능 추가, 코드 리팩토링을 꾸준히 할 예정입니다.**
